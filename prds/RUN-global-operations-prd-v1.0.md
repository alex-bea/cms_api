...
- **If you see `status=manifest_json` or `status=missing_target`:** repair the affected dataset(s) before MPFS runs (capture a CSV backup under `/tmp`). Export `SNAPSHOT_SEARCH_ROOTS=/var/data/ingestion/production/curated` and pass `--use-latest-drop` so the CLI automatically rewrites missing releases to the newest curated drop on disk:
  ```bash
