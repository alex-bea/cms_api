#!/usr/bin/env python3
"""Audit dataset snapshot paths and manifest resolution.

Usage:
    python tools/audit_snapshot_paths.py [--dataset-id rvu_items] [--limit 100]

The script inspects entries in the dataset_snapshots table, highlights
snapshots whose manifest URLs still point to manifest.json files, and verifies
that the DatasetSnapshotService can resolve each snapshot to a concrete parquet
path on disk.
"""
...
def resolve_snapshot_path(service: DatasetSnapshotService, snapshot: DatasetSnapshot) -> Optional[Path]:
...
def audit_snapshots(dataset_id: str | None, limit: int, show_all: bool) -> int:
    ...
    try:
        ...
        ).limit(limit if limit > 0 else None).all()
...
def main() -> int:
    ...
    parser.add_argument("--limit", type=int, default=100, help="Maximum snapshots to inspect (default: 100)")
    parser.add_argument(
        "--show-all",
        action="store_true",
