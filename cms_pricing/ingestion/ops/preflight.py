...
def main() -> int:
    ...
    parser.add_argument(
        "--log-path",
        type=Path,
        help="Optional log path (default: /tmp/preflight/<timestamp>.log)",
    )

    args = parser.parse_args()
    datasets = args.dataset_ids or ["rvu_items", "gpci_indices"]
    timestamp = datetime.utcnow().strftime("%Y%m%d_%H%M%S")
    default_log_dir = Path("/tmp/preflight")
    log_path = args.log_path or default_log_dir / f"snapshot_preflight_{timestamp}.log"
